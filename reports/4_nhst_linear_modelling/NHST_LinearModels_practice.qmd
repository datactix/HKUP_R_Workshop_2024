...
Expr <- read.csv(file = "../../data/curated_datasets/HKUIP_MI_Nanostring-eCRF_Curated.tsv",
                 sep = "\t", header = T) %>% 
        select(-any_of(var_to_remove))

# Load facs data and the annotation
facs <- read.csv(file = "../../data/curated_datasets/HKUIP_MI_Cytometry-eCRF_Curated.tsv",
                 sep = "\t", header = T) %>% 
        select(-any_of(var_to_remove)) %>% 
        rename_with(~ gsub("\\.panel\\d*", "", .), everything()) %>% 
        rename_with(~ gsub("^N_", "", .), everything())
facs_annot <- read.csv(file = "../../data/facs_annotation.tsv", sep = "\t", header = T)

# Load donors' information
MIdonors <- read.csv(file = "../../data/curated_datasets/HKUIP_MI_eCRF_Curated.tsv", 
                     sep = "\t", header = T) %>% 
            select(-any_of(var_to_remove))
```
...
**Bonus:** What are the cells that are significantly differently represented between the smoking groups of MI? Are these cells more related to the innate or the adaptive immune response?

```{r}

```

Fit a single model to compare the total number of cells between sex. Have a look at the normality of the residuals, what can you say?
...
#### (b) Kruskal-Wallis (bonus)

Perform a Kruskall-Wallis test to make the same analysis as with the ANOVA but with a more apropriate test.
...
## 2.5 General Linear Models (GLM)

### Perform a logistic regression

Here is a visualization of the CMV serostatus (1 : Positive, 0 : Negative) as a function of CD4+ EMRA counts.

![](images/clipboard-683486668.png)

Perform a logistic regression to test for the association between the count of CD4+ EMRA cells and the CMV serostatus of the MI donors using `glm`.
