...
data <- data.frame(# Simulate different proportions of male and females
                   ...
                   # Simulate a numeric variable called metabolic score 
                   # (no normal distribution)
                   # Metabolic.score = c(rgamma(n_female, shape = 9, rate = .5), 
                   #                     rgamma(n_male, shape = 1, rate = 2)), 
                   Metabolic.score = c(rgamma(n_female, shape = 9, rate = 1.5), 
...
We're not going to details all the theory behind it but there is a key mathematical principle that should be mentioned: the central limit theorem. This theorem basically tells that if we have a large enough amount of samples, the mean of the subset population follows a normal distribution.
...
Note that you can compute a confidence interval for every parameter. Here is the IC formula of the variance : $IC_{1-\alpha}(\sigma^2) = \left[\frac{(n-1)s^2}{a};\frac{(n-1)s^2}{b}\right]$ where $a = \chi^2_{1-\alpha/2, n-1}$ and $b = \chi^2_{\alpha/2, n-1}$ are the quantiles of the $\chi^2_{n-1}$ distribution.
...
In no case we could have $H_A : \mu_A \geq \mu_B$ because a part of $H_A$ is included in $H_0$.

When a statistical test is performed, it computes the probability of observing our data given that $H_0$ is true. This probability is what we called the $p$-value. Given a level of confidence $1-\alpha$ , we reject $H_0$ if the $p$-value is lower than $\alpha$, and therefore consider that $H_A$ is true.
...
If needed the test can be one sided and the alternative hypotheis will be either $H_A : \mu_A > \mu_B$ (greater) or $H_A : \mu_A < \mu_B$ (less).

*Some theory*

The statistic of the $t$-test to compare means is given by

$$
S = \frac{\bar{X}_A - \bar{X}_B}{\sqrt{\frac{\bar{\sigma}_A^2}{n_A} + \frac{\bar{\sigma}_B^2}{n_B} }} \sim \mathcal{T}_{n-2}
$$

The $p$-value then depends of the value of the statistic $S$ and corresponds to the area under the curve starting from $S$ as showed in the following figure.
...
Example of the variable `Metabolic.score` in the data that was generated using the gamma distribution.

```{r, echo = F}
data %>% 
  pull(Metabolic.score) %>% 
  ggqqplot()

data %>% 
  ggplot(aes(x = Metabolic.score)) +
  geom_density(alpha = .4, fill = "blue") +
  theme_bw() + 
  xlab("Metabolic Score") + 
  ylab("Density")
```
...
Even if this test is designed to be performed on non-normal distributed data, it also works on normally distributed data.

We show here an example on the metabolic score in the generated data. These data *a priori* do not follow a normal distribution. For that we use the function `wilcox.test` that has a similar structure as `t.test`.
...
In the previous section, you have seen how to compare two distinct groups for a given variable. But what happens when you have more than 2 groups? For example in your experiment, instead of having Healthy/Disease, you have 3 groups Asymptomatic, Mild and Severe. Can you apply a $t$-test to compare them all together? The answer is no. you can perform a pairwise comparison by applying a $t$-test to compare Asymptomatic-Mild, then another to compare Asymptomatic-Severe and finally a test to compare Mild and Severe. Actually the number of tests to perform for a pairwise comparison increases quadratically with the number of groups $\left(\frac{n(n-1)}{2}\right)$.
...
This is for the two sided test, obviously you can change alternative hypothesis for the one sided alternatives.
...
A simple linear model corresponds to the case where we want to find the linear relationship between $y$ and a single explanatory variable $x$. In our case, we want to compare the expression of one of the genes to the metabolic score.
...
So here, the first step is to visualize the expression of Gene2 as a function of the metabolic score.

```{r, echo = F}
data %>% 
  ggplot(aes(x = Metabolic.score, y = Gene2)) +
  geom_point() + 
  theme_bw() + # Optional to change the theme of the plot
  xlab("Metabolic score") # Optional change the x axis title
```

In this plot we notice that the expression of Gene2 seems to increase with the metabolic score. However, the relationship between the two variables does not seem to be "linear", instead it appears to be more a "logartithmic" relationship.

```{r, echo = F}
# data %>% lm(Gene2 ~ log(Metabolic.score), data = .) %>% summary()
...
In this kind of situation, we can still fall back to a linear relationship if we log-transform the metabolic score.

```{r, echo = F}
data.log <- data %>% 
            mutate(logMS = log(Metabolic.score)) 
...
Now that we have log-transformed our data, we can assume that there is a linear relationship between the metabolic score and the expression of Gene2. For this purpose, we will use the function `lm` and use the *formula* syntax `y ~ x`. Then we use the function `summary` to get useful information regarding our model.
...
A nice way to visualize the interaction between a continuous variable and a categorical variable is to combine `geom_smooth` with the color parameter and see eventual different lines. For example if we were to study the interaction between the metabolic score and sex to explain the expression of Gene2, we could do as follows.
...
![](images/Capture%20d’écran%202024-11-08%20à%2012.56.17.png){width="353"}
...
![](images/Capture%20d’écran%202024-11-08%20à%2012.57.12.png){width="347"}
