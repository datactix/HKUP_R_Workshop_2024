...
*N.B.* To properly compute a confidence interval of the mean, your data do not need to follow a specific distribution. However to assume the normal distribution of the mean, you need enough samples (typically $n > 30$). In the [annexe](./NHST_LinearModels_Annexe.html#how-are-built-ci) we show alternatives in the case of the number of samples is too small.
...
For more information on the difference of the two sided and one sided $t$-test and on the $t$-statistic, you can check [this section](./NHST_LinearModels_Annexe.html#students-t-test)
...
When performing a linear model it is important to check the assumptions of the linear model. You can easily check them by using the base function `plot` on your model. This will return 4 plots:
